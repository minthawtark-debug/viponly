      ...
      <header className="border-b border-gold/20 bg-background/80 backdrop-blur-sm">
        <div className="container mx-auto flex items-center justify-between px-4 py-4">
          ...
          <nav className="flex gap-4">
            <Link to="/members" className="text-muted-foreground transition-colors hover:text-gold">Members</Link>
            <Link to="/vip" className="font-medium text-gold">VIP</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center">
            <p className="text-muted-foreground">No VIP members to display yet.</p>
          </div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {members.map((member) => (
